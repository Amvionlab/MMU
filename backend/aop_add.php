...
if ($action == "getAllLocations") {
    ...
    $sql = "SELECT id, name, is_active FROM aop";
    ...
    $getStatusSql = "SELECT is_active FROM aop WHERE id = ?";
    ...
    $updateStatusSql = "UPDATE aop SET is_active = ? WHERE id = ?";
    ...
    $checkSql = "SELECT id FROM aop WHERE name = ?";
    ...
    if ($stmt->num_rows > 0) {
        ...
        $insertSql = "INSERT INTO aop (name, is_active) VALUES (?, ?)";
        $isActive = 1;
    ...
    $checkSql = "SELECT id FROM aop WHERE name = ? AND id != ?";
    $stmt = $conn->prepare($checkSql);
    ...
    if ($stmt->num_rows > 0) {
        ...
        $updateSql = "UPDATE aop SET name = ? WHERE id = ?";
