...
if ($action == "getAllLocations") {
    ...
    $sql = "SELECT id, name, is_active FROM territory";
    ...
    $getStatusSql = "SELECT is_active FROM territory WHERE id = ?";
    ...
    $updateStatusSql = "UPDATE territory SET is_active = ? WHERE id = ?";
    ...
    $checkSql = "SELECT id FROM territory WHERE name = ?";
    ...
    if ($stmt->num_rows > 0) {
        ...
        $insertSql = "INSERT INTO territory (name, is_active) VALUES (?, ?)";
        $isActive = 1;
    ...
    $checkSql = "SELECT id FROM territory WHERE name = ? AND id != ?";
    $stmt = $conn->prepare($checkSql);
    ...
    if ($stmt->num_rows > 0) {
        ...
        $updateSql = "UPDATE territory SET name = ? WHERE id = ?";
