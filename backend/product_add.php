...
if ($action == "getAllLocations") {
    ...
    $sql = "SELECT id, name, is_active FROM product";
    ...
    $getStatusSql = "SELECT is_active FROM product WHERE id = ?";
    ...
    $updateStatusSql = "UPDATE product SET is_active = ? WHERE id = ?";
    ...
    $checkSql = "SELECT id FROM product WHERE name = ?";
    ...
    if ($stmt->num_rows > 0) {
        ...
        $insertSql = "INSERT INTO product (name, is_active) VALUES (?, ?)";
        $isActive = 1;
    ...
    $checkSql = "SELECT id FROM product WHERE name = ? AND id != ?";
    $stmt = $conn->prepare($checkSql);
    ...
    if ($stmt->num_rows > 0) {
        ...
        $updateSql = "UPDATE product SET name = ? WHERE id = ?";
