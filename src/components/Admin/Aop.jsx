...
const Form = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${baseURL}/backend/fetchAop.php`);
  ...
  const handleSaveClick = async (id) => {
    try {
      const response = await fetch(`${baseURL}/backend/aop_add.php?action=update`, {
        method: "POST",
  ...
  const handleStatusToggle = async (user) => {
    ...
    try {
      ...
      const response = await fetch(`${baseURL}/backend/aop_add.php?action=toggleStatus`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch(`${baseURL}/backend/aop_add.php?action=getAllLocations`);
      const data = await response.json();
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingId === null) {
      // Add new location
      try {
        const response = await fetch(`${baseURL}/backend/aop_add.php?action=add`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        const result = await response.json();
        if (result.success) {
          toast.success("AOP added successfully.");
          fetchData();
  ...
  const exportCSV = () => {
    ...
    link.setAttribute("download", "AOP.csv");
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(workbook, "AOP.xlsx");
  };
      ...
      {showForm && (
        <div className="max-w-full mb-0.5 p-4 bg-box font-mont">
          <form onSubmit={handleSubmit} className="space-y-4 text-label">
            ...
            <div className="font-mont font-bold text-2xl mb-4 text-center">Add AOP </div>
