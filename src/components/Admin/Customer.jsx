...
const Form = () => {
  ...
  return (
    <div className="bg-second max-h-5/6 max-w-4/6 text-xs mx-auto p-1 lg:overflow-y-hidden h-auto ticket-scroll">
      <div
        className={`box max-w-5xl m-2 mb-4 bg-box p-3 rounded-lg font-mont transition-height duration-300 ${
  ...
  <thead className="bg-prime text-white">
    <tr>
    {["Id", "Name", "Location", "Mobile", "Email", "Department", "Contact_person"].map((header, index) => (
  ...
  <tbody>
    {currentTickets.map((user) => (
      <tr key={user.id} className="hover:bg-gray-100">
        ...
        <td className="border-t py-4 px-4" style={{ textAlign: 'left' }}>{user.name}</td>
        <td className="border-t py-4 px-4" style={{ textAlign: 'left' }}>{user.location}</td>
        <td className="border-t py-4 px-4" style={{ textAlign: 'left' }}>{user.mobile}</td>
        <td className="border-t py-4 px-4" style={{ textAlign: 'left' }}>{user.email}</td>
        <td className="border-t py-4 px-4" style={{ textAlign: 'left' }}>{user.department}</td>
        <td className="border-t py-4 px-4" style={{ textAlign: 'left' }}>{user.contact_person}</td>
      </tr>
    ))}
  </tbody>
</table>
