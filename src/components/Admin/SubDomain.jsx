...
const Form = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(`${baseURL}/backend/fetchSubDomain.php`);
        const data = await response.json();
        setUsers(data);
  ...
  const exportCSV = () => {
    ...
    link.setAttribute("download", "Sub_Domain.csv");
          ...
          <div className="ticket-table mt-2">
            <form onSubmit={handleSubmit} className="space-y-4 text-label">
              ...
              >
                ...
                <button
                  type="submit"
                  className="ml-5 bg-prime font-mont font-semibold text-md text-white py-2 px-2 rounded-md shadow-md focus:outline-none"
                >
                  Submit
                </button>
      ...
      <div className="max-w-5xl m-2 bg-box p-3 rounded-lg font-mont">
       <div className="flex justify-end flex-wrap space-x-2 mt-4">
        ...
        <div className="ticket-table mt-8">
          <h2 className="text-2xl font-bold text-prime mb-4"><span>Sub Domain Data </span><span className="items-end"><button
        ...
        <label htmlFor="rowsPerPage" className="text-sm font-medium text-gray-700">
         ...
         <table className=" min-w-full bg-second rounded-lg overflow-hidden filter-table">
            <thead className="bg-prime text-white">
              <tr>
                {["Id", "name","domain_id"].map((header, index) => (
            ...
            <tbody>
              {currentTickets.map((user) => (
                <tr key={user.id} className="hover:bg-gray-100">
                  ...
                  <td className="border-t py-4 px-4">{user.domain_name}</td>
