...
const Map = () => {
  ...
  const { mmu } = useParams(); // Get the `mmu` parameter from the URL

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          'https://app.gpstrack.in/api/get_current_data?token=projectmmu_7jkITDLjgki2HoR1OLtwqpcHSXPQs6Kz&email=projectmmu@ircstnb.org'
        );
        if (response.ok) {
  ...
  // Match the mmu value (1-based) to the vehicle index (0-based) in the vehicles array
  const selectedVehicle = vehicles[parseInt(mmu, 10) - 1];
  ...
  return (
    <div className="p-0.5">
      <MapContainer center={currentLocation} zoom={6} style={{ height: '625px', width: '100%' }}>
        ...
        {/* Display marker for the selected vehicle */}
        {selectedVehicle ? (
          <Marker
          ...
          >
            <Popup>
              <div>
                ...
                <p>Fuel: {selectedVehicle.fuelLitre} litres</p>
              </div>
            </Popup>
          </Marker>
        ) : (
          <p>Vehicle data not available for MMU value: {mmu}</p>
        )}
      </MapContainer>
    </div>
  );
